...
/// <summary>
/// Renders the throttle pedal input as a vertical bar.
/// </summary>
public class ThrottleInputRenderer
{
    ...
    /// <summary>
    /// Renders the throttle input visualization.
    /// </summary>
    ...
    public void Render(IRenderer renderer, RenderContext context)
    {
        ...
        var throttleValue = Math.Clamp(context.TelemetryData.Throttle, 0f, 1f);
        ...
        // Draw throttle bar (green, bottom to top based on value)
        var filledHeight = BarHeight * throttleValue;
        var filledY = y + BarHeight - filledHeight;
        renderer.DrawRectangle(x, filledY, BarWidth, filledHeight, 0.1f, 0.9f, 0.1f, 0.9f);

        // Draw percentage indicator using small rectangles (since text rendering is not yet implemented)
        DrawPercentageIndicator(renderer, x, y, BarHeight, throttleValue);
    }
